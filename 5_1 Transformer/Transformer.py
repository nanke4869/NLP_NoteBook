...
def get_attn_subsequent_mask(seq):
    ...
    # torch.from_numpy = torch.Tensor: 将numpy类转换成tensor类
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):  # [batch_size x len_q x d_model]
        residual = inputs
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        ...
        # [batch_size x len_q x d_model], attn: [batch_size x n_heads x len_q(=len_k) x len_k(=len_q)]
...
class Encoder(nn.Module):
    def __init__(self, src_vocab_size, src_len, d_model, d_k, d_v, n_heads, n_layers, d_ff):
...
class Decoder(nn.Module):
    def __init__(self, tgt_vocab_size, tgt_len, d_model, d_k, d_v, n_heads, n_layers, d_ff):
...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, src_len, d_model, d_k, d_v, n_heads, n_layers, tgt_vocab_size, tgt_len):
        super(Transformer, self).__init__()
        self.encoder = Encoder(src_vocab_size, src_len, d_model, d_k, d_v, n_heads, n_layers, d_ff)
        self.decoder = Decoder(tgt_vocab_size, tgt_len, d_model, d_k, d_v, n_heads, n_layers, d_ff)
...
if __name__ == '__main__':
    ...
    src_vocab = {'P': 0, 'ich': 1, 'mochte': 1, 'ein': 3, 'bier': 4}
    ...
    model = Transformer(src_vocab_size, src_len, d_model, d_k, d_v, n_heads, n_layers, tgt_vocab_size, tgt_len)
    ...
    predict, enc_self_attns, dec_self_attns, dec_enc_attns = model(enc_inputs, greedy_dec_input)
